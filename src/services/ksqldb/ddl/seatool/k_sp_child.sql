...
CREATE TABLE IF NOT EXISTS K_seatool_agg_Components_StatePlans
  WITH (KAFKA_TOPIC='${param:topicNamespace}aws.ksqldb.seatool.agg.Components_StatePlans',KEY_FORMAT='JSON',WRAP_SINGLE_VALUE=FALSE) AS
...
CREATE TABLE IF NOT EXISTS K_seatool_agg_State_Plan_1115
  WITH (KAFKA_TOPIC='${param:topicNamespace}aws.ksqldb.seatool.agg.State_Plan_1115',KEY_FORMAT='JSON',WRAP_SINGLE_VALUE=FALSE) AS
  SELECT sp.ID_Number,
         STRUCT(
...
CREATE TABLE IF NOT EXISTS K_seatool_agg_State_Plan_APD_Sub_Type
  WITH (KAFKA_TOPIC='${param:topicNamespace}aws.ksqldb.seatool.agg.State_Plan_APD_Sub_Type',KEY_FORMAT='JSON',WRAP_SINGLE_VALUE=FALSE) AS
  SELECT sp.pk->payload->ID_Number,
...
CREATE TABLE IF NOT EXISTS K_seatool_agg_State_Plan_Early_Alerts
  WITH (KAFKA_TOPIC='${param:topicNamespace}aws.ksqldb.seatool.agg.State_Plan_Early_Alerts',KEY_FORMAT='JSON',WRAP_SINGLE_VALUE=FALSE) AS
  SELECT sp.pk->payload->ID_Number,
         STRUCT(
...
CREATE TABLE IF NOT EXISTS K_seatool_agg_RAI
  WITH (KAFKA_TOPIC='${param:topicNamespace}aws.ksqldb.seatool.agg.RAI',KEY_FORMAT='JSON',WRAP_SINGLE_VALUE=FALSE) AS
...
CREATE TABLE IF NOT EXISTS K_seatool_agg_State_Plan_APD
  WITH (KAFKA_TOPIC='${param:topicNamespace}aws.ksqldb.seatool.agg.State_Plan_APD',KEY_FORMAT='JSON',WRAP_SINGLE_VALUE=FALSE) AS
  SELECT sp.ID_Number,
...
CREATE TABLE IF NOT EXISTS K_seatool_agg_State_Plan_Impact_Funding
  WITH (KAFKA_TOPIC='${param:topicNamespace}aws.ksqldb.seatool.agg.State_Plan_Impact_Funding',KEY_FORMAT='JSON',WRAP_SINGLE_VALUE=FALSE) AS
  SELECT sp.ID_Number,
...
CREATE TABLE IF NOT EXISTS K_seatool_agg_Stop_Resume_Dates
  WITH (KAFKA_TOPIC='${param:topicNamespace}aws.ksqldb.seatool.agg.Stop_Resume_Dates',KEY_FORMAT='JSON',WRAP_SINGLE_VALUE=FALSE) AS
...
CREATE TABLE IF NOT EXISTS K_seatool_agg_State_Plan_Service_Types
  WITH (KAFKA_TOPIC='${param:topicNamespace}aws.ksqldb.seatool.agg.State_Plan_Service_Types',KEY_FORMAT='JSON',WRAP_SINGLE_VALUE=FALSE) AS
  SELECT st.ID_Number,
         COLLECT_LIST(st.StatePlanServiceType) as StatePlanServiceTypes
    FROM  K_seatool_tld_State_Plan_Service_Types st
  GROUP BY st.ID_Number
EMIT CHANGES;


CREATE TABLE IF NOT EXISTS K_seatool_agg_State_Plan_Service_SubTypes
  WITH (KAFKA_TOPIC='${param:topicNamespace}aws.ksqldb.seatool.agg.State_Plan_Service_SubTypes',KEY_FORMAT='JSON',WRAP_SINGLE_VALUE=FALSE) AS
  SELECT st.pk->payload->ID_Number,
         COLLECT_LIST(st.StatePlanServiceSubType) as StatePlanServiceSubTypes
    FROM  K_seatool_tld_State_Plan_Service_SubTypes st
  GROUP BY st.pk->payload->ID_Number
EMIT CHANGES;
